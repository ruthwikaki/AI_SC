...
class SupplierInterface:
    ...
    async def get_average_supplier_performance(self, start_date: date, end_date: date) -> float:
        """
        Get average supplier performance score for a given period.
        ...
        Returns:
            Average performance score as a percentage (0-100)
        """
        try:
            ...
            if not supplier_table:
                ...
                return 87.2  # Default value
            
            # Get supplier columns
            supplier_columns = self._get_columns_for_table(mappings, supplier_table, "supplier")
            
            # Look for performance score column
            performance_col = supplier_columns.get('performance_score', 
                            supplier_columns.get('rating', 
                            supplier_columns.get('score', 'performance_score')))
            
            # Try to find supplier performance table
            performance_table = self._find_table_by_concept(mappings, "supplier_performance")
            
            if performance_table:
                # Use performance table if available
                perf_columns = self._get_columns_for_table(mappings, performance_table, "supplier_performance")
                supplier_col = perf_columns.get('supplier', 'supplier_id')
                score_col = perf_columns.get('score', 'performance_score')
                date_col = perf_columns.get('evaluation_date', 'date')
                
                query = f"""
                    SELECT AVG({score_col}) as avg_performance
                    FROM {performance_table}
                    WHERE {date_col} BETWEEN :start_date AND :end_date
                """
            else:
                # Use supplier table with performance column
                query = f"""
                    SELECT AVG({performance_col}) as avg_performance
                    FROM {supplier_table}
            ...
            if result["data"] and result["data"][0].get("avg_performance") is not None:
                avg = float(result["data"][0]["avg_performance"])
                # If the score is 0-1, convert to percentage
                if avg <= 1:
                    avg = avg * 100
                return avg
